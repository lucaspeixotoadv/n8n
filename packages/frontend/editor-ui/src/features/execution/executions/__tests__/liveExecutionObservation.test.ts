...
/** The store's own signature, so the mocks answer exactly what the handlers await. */
type FetchExecutionDataById = ReturnType<typeof useWorkflowsStore>['fetchExecutionDataById'];
...
describe('observing a running execution', () => {
	...
	beforeEach(() => {
		...
		workflowsStore.fetchExecutionDataById = vi.fn<FetchExecutionDataById>(
	...
	describe('switching', () => {
		...
		it('keeps showing the next execution when the previous one finishes during the switch', async () => {
			...
			let answerFinish: (value: IExecutionResponse | null) => void = () => {};
			workflowsStore.fetchExecutionDataById = vi.fn<FetchExecutionDataById>(
				async () =>
					await new Promise<IExecutionResponse | null>((resolve) => (answerFinish = resolve)),
			);
			...
			answerFinish(stored.get('42') ?? null);
			await settle();
